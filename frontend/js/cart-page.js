...
function renderCart() {
    ...
    if (cart.length === 0) {
        container.innerHTML = `
            <div class="text-center py-5 text-muted fade-in-up">
                ...
                <a href="home.html" class="btn btn-premium mt-3 rounded-pill px-4 shadow-sm hover-scale">Continue Shopping</a>
            </div>
        `;
        cartSubContainer.classList.add('d-none');
        document.getElementById('buyBoxSubtotalText').textContent = 'Subtotal (0 items):';
        document.getElementById('buyBoxSubtotalMain').textContent = '₹0';
        document.getElementById('proceedCheckoutBtn').classList.add('disabled');
        return;
    }

    cartSubContainer.classList.remove('d-none');
    document.getElementById('proceedCheckoutBtn').classList.remove('disabled');
                ...
                <div class="col-md-8 col-8">
                    ...
                    <div class="d-flex align-items-center gap-3 mt-3">
                        <div class="input-group input-group-sm rounded-pill shadow-sm overflow-hidden" style="width: 110px; border: 1px solid #D5D9D9;">
                            ...
                            <input type="text" class="form-control text-center border-0 bg-light fw-bold" value="${item.quantity}" readonly>
                        ...
                        <div class="vr bg-light"></div>
                        <a href="#" class="text-primary small text-decoration-none hover-scale">Save</a>
                    </div>
                </div>
                <div class="col-md-2 col-12 text-md-end mt-3 mt-md-0">
                    <span class="fw-bold fs-5 text-dark">₹${(item.price * item.quantity).toLocaleString('en-IN')}</span>
                </div>
            </div>
        `;
    });
    ...
    document.getElementById('cartSubtotalText').textContent = subText + ' ';
    document.getElementById('cartSubtotalMain').textContent = priceText;
    document.getElementById('buyBoxSubtotalText').textContent = subText;
    document.getElementById('buyBoxSubtotalMain').textContent = priceText;
}
